...
class SignalPlot(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Leyenda — apilada verticalmente en la esquina superior derecha
        painter.setFont(QFont("Segoe UI", 8))
        fm = painter.fontMetrics()
        line_h = fm.height() + 4
        for idx, buf in enumerate(self.buffers):
            color = QColor(BUFFER_COLORS[idx % len(BUFFER_COLORS)])
            label = f"Buffer {idx + 1}  ({len(buf)} muestras)"
            lbl_w = fm.horizontalAdvance(label)
            lx = w - pad - lbl_w - 22
            ly = pad + idx * line_h + line_h

            # Fondo de la leyenda
            painter.fillRect(lx - 4, ly - fm.height() - 2, lbl_w + 30, line_h, QColor("#ffffffcc"))

            painter.setPen(QPen(color, 2))
            painter.drawLine(lx, ly - 4, lx + 16, ly - 4)
            painter.setPen(QColor("#333333"))
            painter.drawText(lx + 20, ly, label)
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        btn_clear_plot.setFixedWidth(150)
